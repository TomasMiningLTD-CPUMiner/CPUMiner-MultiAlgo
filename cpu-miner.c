...
json_t *json_rpc2_call_recur(CURL *curl, const char *url,
	...
	if(recur >= 5) {
		if(opt_debug)
			applog(LOG_DEBUG, "Nie uda³o siê wywo³aæ komendy rpc po %i próbach", recur);
		return NULL;
	}
	if(!strcmp(rpc2_id, "")) {
		if(opt_debug)
			applog(LOG_DEBUG, "Próba po³¹czenia komendy RPC2 przed uwierzytelnianiem");
		return NULL;
	}
	...
	if(!strcmp(mes, "Unauthenticated")) {
		...
		applog(LOG_ERR, "json_rpc2.0 b³¹d: %s", mes);
		return NULL;
	}
	end:
	return res;
...
static bool jobj_binary(const json_t *obj, const char *key, void *buf,
    ...
    if (unlikely(!tmp)) {
        applog(LOG_ERR, "klucz JSON '%s' nie znaleziony", key);
        return false;
    }
    hexstr = json_string_value(tmp);
    if (unlikely(!hexstr)) {
        applog(LOG_ERR, "klucz JSON '%s' nie jest ³añcuchem", key);
        return false;
    }
...
bool rpc2_job_decode(const json_t *job, struct work *work) {
    ...
    if (blobLen != 0) {
        ...
        if(rpc2_target != target) {
            ...
            applog(LOG_INFO, "Pool ustawi³ trudnosæ na %g", difficulty);
            rpc2_target = target;
        }
...
static bool work_decode(const json_t *val, struct work *work) {
    ...
    if (unlikely(!jobj_binary(val, "data", work->data, sizeof(work->data)))) {
        applog(LOG_ERR, "JSON nieprawid³owe dane");
        goto err_out;
    }
    if (unlikely(!jobj_binary(val, "target", work->target, sizeof(work->target)))) {
        applog(LOG_ERR, "JSON nieprawid³owy cel");
        goto err_out;
    }
...
bool rpc2_login_decode(const json_t *val) {
    ...
    if(!res) {
        applog(LOG_ERR, "JSON nieprawid³owy wynik");
        goto err_out;
    }
    ...
    if(!tmp) {
        applog(LOG_ERR, "JSON nieprawid³owy identyfikator");
        goto err_out;
    }
    id = json_string_value(tmp);
    if(!id) {
        applog(LOG_ERR, "JSON id nie jest ci¹giem znaków");
        goto err_out;
    }
    ...
    if(!tmp) {
        applog(LOG_ERR, "JSON nieprawid³owy status");
        goto err_out;
    }
    ...
    if(strcmp(s, "OK")) {
        applog(LOG_ERR, "JSON  zwrócony Status \"%s\"", s);
        return false;
    }
...
static void share_result(int result, struct work *work, const char *reason) {
    ...
    case ALGO_CRYPTONIGHT:
        applog(LOG_INFO, "zaakceptowano: %lu/%lu (%.2f%%), %.2f H/s at diff %g %s",
    ...
    default:
        ...
        applog(LOG_INFO, "zaakceptowano: %lu/%lu (%.2f%%), %s khash/s %s",
                accepted_count, accepted_count + rejected_count,
    ...
    if (opt_debug && reason)
        applog(LOG_DEBUG, "DEBUG: powód odrzucenia: %s", reason);
}
...
static bool submit_upstream_work(CURL *curl, struct work *work) {
    ...
    if (!submit_old && memcmp(work->data + 1, g_work.data + 1, 32)) {
        if (opt_debug)
            applog(LOG_DEBUG, "DEBUG: stale work detected, odrzucam");
        return true;
    }
...
static bool get_upstream_work(CURL *curl, struct work *work) {
    ...
    if (opt_debug && rc) {
        ...
        applog(LOG_DEBUG, "DEBUG: dosta³ now¹ pracê w %d ms",
...
//    applog(LOG_DEBUG, "JSON value: %s", json_dumps(val, 0));
    ...
    if (opt_debug && rc) {
        timeval_subtract(&diff, &tv_end, &tv_start);
        applog(LOG_DEBUG, "DEBUG: autoryzacja w %d ms",
                diff.tv_sec * 1000 + diff.tv_usec / 1000);
    }
...
static bool workio_get_work(struct workio_cmd *wc, CURL *curl) {
    ...
    while (!get_upstream_work(curl, ret_work)) {
        ...
        applog(LOG_ERR, "getwork nie powiod³o siê, spróbuj ponownie po %d sekundach",
                opt_fail_pause);
...
static bool workio_submit_work(struct workio_cmd *wc, CURL *curl) {
    ...
    while (!submit_upstream_work(curl, wc->u.work)) {
        if (unlikely((opt_retries >= 0) && (++failures > opt_retries))) {
            applog(LOG_ERR, "...zakoñczenie w¹tku roboczego");
            return false;
        }
...
static bool workio_login(CURL *curl) {
    ...
    while (!rpc2_login(curl)) {
        ...
        /* pause, then restart work-request loop */
        applog(LOG_ERR, "...retry after %d sekundach", opt_fail_pause);
        sleep(opt_fail_pause);
        pthread_mutex_unlock(&rpc2_login_lock);
        pthread_mutex_lock(&rpc2_login_lock);
    }
    ...
    while (1) {
        ...
        if (jsonrpc_2) {
            if (stratum.work.job_id
                ...
                time(&g_work_time);
                pthread_mutex_unlock(&g_work_lock);
                applog(LOG_INFO, "Stratum wykry³o nowy blok");
                restart_threads();
            }
        } else {
        ...
        if (!stratum_socket_full(&stratum, 120)) {
            applog(LOG_ERR, "Up³yn¹³ limit czasu po³¹czenia do Stratum");
            s = NULL;
        } else
            s = stratum_recv_line(&stratum);
        if (!s) {
            stratum_disconnect(&stratum);
            applog(LOG_ERR, "Przerwano po³¹czenie do Stratum");
            continue;
        }
